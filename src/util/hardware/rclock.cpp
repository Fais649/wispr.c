...
void RClock::getTime(m5::rtc_time_t &time)
{
    rtc->getTime(&time);
}

void RClock::getDate(m5::rtc_date_t &date)
{
    rtc->getDate(&date);
}

void RClock::setTime(const m5::rtc_time_t &time)
{
    rtc->setTime(&time);
}

void RClock::setDate(const m5::rtc_date_t &date)
{
    rtc->setDate(&date);
}

void RClock::setInitialTime(const m5::rtc_time_t &time, const m5::rtc_date_t &date)
{
    rtc->setTime(&time);
    rtc->setDate(&date);
}
